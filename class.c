...
int class_getnoperands(U1 instruction) { return noperands[instruction]; }
...
char *class_getclassname(ClassFile *class, U2 index) {
  return class_getutf8(class,
                       class->constant_pool[index]->info.class_info.name_index);
}

/* get string from string reference */
char *class_getstring(ClassFile *class, U2 index) {
  return class_getutf8(
...
void class_getnameandtype(ClassFile *class, U2 index, char **name,
                          char **type) {
  *name = class_getutf8(
      class, class->constant_pool[index]->info.nameandtype_info.name_index);
  *type = class_getutf8(
      class,
      class->constant_pool[index]->info.nameandtype_info.descriptor_index);
}
...
Method *class_getmethod(ClassFile *class, char *name, char *descr) {
  ...
  for (i = 0; i < class->methods_count; i++)
    if (strcmp(name, class_getutf8(class, class->methods[i]->name_index)) ==
            0 &&
        strcmp(descr,
               class_getutf8(class, class->methods[i]->descriptor_index)) == 0)
...
Field *class_getfield(ClassFile *class, char *name, char *descr) {
  ...
  for (i = 0; i < class->fields_count; i++)
    if (strcmp(name, class_getutf8(class, class->fields[i]->name_index)) == 0 &&
        strcmp(descr,
               class_getutf8(class, class->fields[i]->descriptor_index)) == 0)
...
int class_istopclass(ClassFile *class) {
  return strcmp(class_getclassname(class, class->super_class),
                "java/lang/Object") == 0;
}
